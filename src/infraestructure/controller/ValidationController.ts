...
export class ValidationController {
  constructor(private readonly validationUseCase: ValidationUseCase) { }

  async validateQuestion(req: Request, res: Response, next: NextFunction) {
    try {
  ...
  async getValidationsByQuestion(req: Request, res: Response, next: NextFunction) {
    try {
  ...
  async getMyValidations(req: Request, res: Response, next: NextFunction) {
    try {
      const validatorId = (req as any).user?.id;
      if (!validatorId) {
        throw new UnauthorizedError('No user found in request');
